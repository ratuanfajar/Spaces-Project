...
<body class="font-sans antialiased bg-slate-950 text-slate-300">
    ...
    <div x-data="{ sidebarOpen: false, sidebarExpanded: true }" class="flex h-screen overflow-hidden">

        <aside 
            class="absolute inset-y-0 left-0 z-50 flex flex-col transition-all duration-300 ease-in-out bg-slate-900 border-r border-slate-800 lg:static lg:translate-x-0"
            :class="{
                '-translate-x-full': !sidebarOpen,   /* Mobile: Hidden default */
                'translate-x-0': sidebarOpen,        /* Mobile: Open */
                'w-64': sidebarExpanded,             /* Desktop: Lebar Penuh */
                'w-20': !sidebarExpanded             /* Desktop: Icon Only */
            }"
        >
            <div class="flex items-center justify-center h-20 border-b border-slate-800/50" 
                 :class="sidebarExpanded ? 'px-6' : 'px-0'">
                <div class="flex-shrink-0 w-10 h-10 bg-white rounded-full flex items-center justify-center text-slate-900">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                </div>
                <span class="ml-3 text-xl font-bold text-white transition-opacity duration-200"
                    ...
                    <a href="{{ Route::has($menu['route']) ? route($menu['route']) : '#' }}" 
                       ...
                       {{ request()->routeIs($menu['route']) ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white' }}">
                        ...
                        <span class="ml-3 font-medium whitespace-nowrap transition-all duration-200"
                              :class="sidebarExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10 absolute pointer-events-none'">
                            {{ $menu['name'] }}
                        </span>

                        <div x-show="!sidebarExpanded" 
                             class="absolute left-full top-1/2 -translate-y-1/2 ml-2 px-2 py-1 bg-slate-700 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none">
                            {{ $menu['name'] }}
                        </div>
                    </a>
                @endforeach
            ...
            <div class="border-t border-slate-800 p-4">
                <div class="flex items-center justify-between" :class="!sidebarExpanded ? 'justify-center' : ''">
                    
                    <div class="flex items-center">
                        <div class="w-9 h-9 rounded-full bg-indigo-500 flex items-center justify-center text-white font-bold text-sm">
                            {{ substr(Auth::user()->name, 0, 1) }}
                        </div>
                        <div class="ml-3" x-show="sidebarExpanded">
            ...
            <button @click="sidebarExpanded = !sidebarExpanded" 
                    class="hidden lg:flex absolute -right-3 top-20 bg-slate-800 border border-slate-700 text-slate-400 hover:text-white rounded-full p-1 shadow-md transition-transform"
        ...
        <div class="flex-1 flex flex-col overflow-hidden relative">
            ...
            <header class="flex items-center justify-between p-4 bg-slate-900 border-b border-slate-800 lg:hidden">
                <div class="flex items-center">
                    ...
                    <span class="ml-4 text-lg font-bold text-white">Spacer</span>
                </div>
            </header>
